...
\begin{document}
    ...
    \begin{frame}[fragile]{Цикл по заданному интервалу}
        \begin{itemize}
            ...
            \item[\answer] \pause Ну будем поддерживать в \mintinline{pythona}{state} кортеж
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}[fragile]{Примитивная рекурсия в $\lambda$-исчислении}
        ...
        \end{block}
        \begin{itemize}
            \item Хотим выразить в виде $s \ap (s \ap (s \ap z))$, какими будут $s$ и $z$?
            ...
            \item Одна итерация:\footnote{Пусть сокращение $\term{pair} \ap i \ap res$ слева значит: принимаем пару $p$, вместо $i$ подразумеваем $\term{fst}\ap p$; $res$ --- $\term{snd}\ap p$.}
            \[
