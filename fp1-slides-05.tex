...
\begin{document}
    ...
    \begin{frame}[fragile]{Зачем встроенные структуры данных в Haskell}
        \begin{itemize}
            ...
            \end{itemize}
            \item Однако эти реализации не всегда годятся на практике:
            \begin{itemize}
    ...
    \begin{frame}[fragile]{Задачки на рекурсивные типы}
        ...
        \begin{itemize}
            ...
            \item[\eg] \mintinline{haskell}{Node (Leaf 1) 2 (Node (Leaf 3) 4 (Leaf 5))}
            \item[\todo] Вычислите глубину дерева и поредуцируйте на примере
            \item[\todo] Вычислите сумму значений и поредуцируйте на примере
            \item[\todo] Примените к каждому значению в дереве функцию и поредуцируйте на примере
            \item[\answer] \pause
            \begin{minted}{haskell}
                height :: Tree -> Int
            ...
            \begin{minted}{haskell}
                ...
                transform f tree = case tree of
                  Leaf x -> Leaf (f x)
                  Node l x r -> Node (transform f l) (f x) (transform f r)
            \end{minted}
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}[fragile]{Полиморфные алгебраические типы}
        \begin{itemize}
            ...
            \item[\todo] Закодируйте полиморфное бинарное дерево и функцию трансформации значений
            \item[\answer] \pause
            \begin{minted}{haskell}
    ...
    \begin{frame}[fragile]{Система кайндов}
        ...
        \begin{itemize}
            ...
            \end{minted}
            \item[\answer] \pause \mintinline{haskell}{Tree :: * -> *}
    ...
    \begin{frame}[fragile]{Очевидная тотальность \practicalslide}
        \begin{itemize}
            \item[\defi] \vocab{Тотальный код} --- никогда не расходится (зацикливания, исключения)
            \begin{itemize}
                \item Но завершаемость проверить нельзя (halting problem)
                \item И исключительные ситуации случаются (ошибки программиста, работа с IO)
            \end{itemize}
            \item Вместо этого используем ``\vocab{очевидную тотальность}''
            \begin{itemize}
            ...
            \end{itemize}
            \item На ошибке программиста --- всё же fail fast\footnote{\color{blue} \url{https://en.wikipedia.org/wiki/Fail-fast}}
            \item Выстраиваем чёткую границу с внешним миром\footnote{\underline{\color{blue} \href{https://lexi-lambda.github.io/blog/2019/11/05/parse-don-t-validate/}{Parse, don’t validate}}}
            \begin{itemize}
    ...
    \sectionplan{Всякое разное \practicalslide}
    ...
    \begin{frame}[fragile]{Полиморфные типы demystified или система $\lambda \omega$ \advancedslide\popslide}
        \begin{itemize}
            ...
            \begin{align*}
                &\term{Pair} : * \rightarrow * \rightarrow * \\
                &\term{Pair} = \lambda \tau^*~\sigma^*\ldotp\forall \gamma\ldotp(\tau\rightarrow\sigma\rightarrow\gamma)\to\gamma \\
                &\term{pair} : \forall \alpha~\beta\ldotp\alpha \rightarrow \beta \rightarrow \term{Pair}~\alpha~\beta \\
                &\term{pair} = \Lambda \alpha^*~\beta^*\ldotp\lambda x^\alpha~y^\beta\ldotp(\Lambda \gamma^*\ldotp\lambda f^{\alpha\rightarrow\beta\rightarrow\gamma}\ldotp f~x~y) \\
                &\term{fst} : \forall \alpha~\beta\ldotp \term{Pair}~\alpha~\beta\rightarrow \alpha \\
                &\term{fst} = \Lambda \alpha^*~\beta^*\ldotp\lambda p^{\term{Pair}~\alpha~\beta}\ldotp p~\alpha~(\mathbf{K}\ap\alpha\ap\beta)
            \end{align*}
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Что посмотреть в транспорте}
        \begin{itemize}
            ...
            \item \href{https://youtu.be/qurG_J81_Cs?si=tINacE3c6ZF3CkvF}{\color{blue} (youtube) Тагир Валеев — Pattern matching в Java и его воображаемые друзья}
            \item \href{https://codewords.recurse.com/issues/three/algebra-and-calculus-of-algebraic-data-types}{\color{blue}  The algebra (and calculus!) of algebraic data types}
        \end{itemize}
    \end{frame}
