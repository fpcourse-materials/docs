...
\begin{document}
    ...
    \begin{frame}[fragile]{Пример программы в $\lambda$-исчислении}
    ...
    \begin{frame}[fragile]{Моделируем котиков (algebra driven design)}
        \begin{itemize}
            \item[\todo] Какое API будет у котиков?
            \item[\answer] \pause Термы $\term{cat}$, $\term{name}$ и $\term{murlicity}$
            \item[\todo] Какие законы будут гарантировать корректность?
            \item[\answer] \pause
            \begin{enumerate}
    ...
    \begin{frame}[fragile]{Кодирование наоборот\footnote{``Наоборот'' также известно как tagless final, \href{https://okmij.org/ftp/tagless-final/course/Boehm-Berarducci.html}{\color{blue} church-boehm-barracudi encoding}, CPS transformation.}}
        \begin{itemize}
    ...
    \begin{frame}[fragile]{Котики в $\lambda$-исчислении}
        ...
        \end{block}
        \begin{itemize}
            \item[\todo] \pause Закодируйте конструктор котиков $\term{cat}$ с помощью идеи ``наоборот''
            \item[\answer] \pause $\term{cat} \termdef \pause \lambda x~y\ldotp \lambda f \ldotp f \ap x \ap y$
            \item Благодаря каррированности, термы вида $\lambda f\ldotp f \ap n \ap m$ можно считать котиками
            \item[\todo] Реализуйте $\term{name}$ и $\term{murlicity}$
            \item[\answer] \pause $\term{name} \termdef \pause \lambda c \ldotp c \ap \term{K}$, $\term{murlicity} \termdef \pause \lambda c \ldotp c \ap \term{K}^*$
            \item Аналогично можно задать интерфейс пар: $\term{pair}$, $\term{fst}$ и $\term{snd}$
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Логические величины в $\lambda$-исчислении}
        ...
        \pause
        \begin{itemize}
            \item[\todo] Закодируйте конструкторы с помощью ``наоборот''
            \item[\answer] \pause $\term{true} \termdef \pause \term{K}$ и $\term{false} \termdef \term{K}^*$
            \item[\todo] Закодируйте $\term{if}$
            \item[\answer] \pause $\term{if} \termdef \pause \lambda b~t~e\ldotp b \ap t \ap e \eqeta \pause \lambda x\ldotp x$
        \end{itemize}
    \end{frame}
