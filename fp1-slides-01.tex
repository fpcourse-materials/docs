...
\begin{document}
    ...
    \begin{frame}{$\beta$-редукция --- применение функций}
        ...
        \pause
        \vspace{1em}
        Этого правила переписывания термов достаточно, чтобы описать любое вычисление!
    \end{frame}
    ...
    \subsectionplan{Дополнительные мета-договорённости}
    ...
    \begin{frame}{\subsecname}
        ...
        \begin{block}{Договорённость о введении синонимичных имён}
            \begin{itemize}
                \item Можем вводить новые термы:
                \[
                ...
                \]
                \item Встречая такие термы, можем их переписывать на определение:
                \[
                    \term{B} \step_{\delta} \lambda f \ap g\ldotp \lambda x\ldotp f~(g~x)
                \]
                \item Аналогично $\beta$-эквивалентности определим $\delta$-эквивалентность: $\eqt_\delta$
            \end{itemize}
        \end{block}
    \end{frame}
    ...
    \begin{frame}[fragile]{``Кодирование наоборот''\footnote{``Наоборот'' также известно как tagless final, \href{https://okmij.org/ftp/tagless-final/course/Boehm-Berarducci.html}{\color{blue} church-boehm-berarducci encoding}, CPS transformation, codata.} \popslide}
        \begin{itemize}
