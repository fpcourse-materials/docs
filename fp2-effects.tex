...
\begin{itemize}
    ...
    Это ограничивает использование эффективных мутабельных продолжений~\ref{subsec:efficient-cps}~\cite{leijen2018algebraic}\footnote{\url{https://koka-lang.github.io/koka/doc/book.html#sec-multi-resume}}.
    ...
    Реализация корутин поверх хендлеров даёт возможность пользователям писать собственные планировщики, что, на данный момент, самое котируемое применение хендлеров на данный момент~\cite{sivaramakrishnan2021retrofitting, phipps2023continuing}.
\end{itemize}
...
Так, мы фактически получим tagless final shallow embedding~\ref{subsec:tagless-final}, другое популярное решение expression problem~--- \vocab{трансформеры монад}~\cite{liang1995monad,jones1995functional}\footnote{\url{https://hackage.haskell.org/package/mtl}}.
В целом эти подходы, равносильны по выразительности, однако могут иметь на практике различные особенности~\cite{schrijvers2019monad}.

% todo show some hint how deforestation gives monad transformers
% todo lift - monad morphism
% todo ContT, lift = (>>=)

%\subsubsection{Алгебраичность и эффекты высших порядков}
