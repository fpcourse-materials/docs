...
\subsubsection{Хендлеры через свободные монады}
...
\begin{minted}{haskell}
    runReader
      :: Functor effs
      => Comp (Reader env |> effs) res -> env -> Comp effs res
...
\begin{minted}{haskell}
    ...
    handle val alg = \case
      Res -> val res
      Op effs -> Op (alg $ handle val alg <$> effs)
\end{minted}
...
% todo what is algebraic about algebraic effects
...
\subsubsection{Трансформеры монад}
