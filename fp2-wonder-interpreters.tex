...
\begin{itemize}
    ...
    \begin{itemize}
        \item Синтаксис языка задаётся набором допустимых нод дерева.
        \item Интерпретатор при виде каждой ноды выполняет соответствующий код на мета-языке (ветку паттерн-матчинга).
    \end{itemize}
    \item С другой:
    \begin{itemize}
        \item Функция задаёт новую ноду дерева: вызов этой функции
        \item Интерпретатор при виде этой ноды выполнят код тела функции.
    \end{itemize}
\end{itemize}
