...
\begin{minted}{haskell}
    data Box f where
      Box :: forall s f . s -> (s -> f s) -> Box f

    data ListF a rec = Nil | Cons a rec
      deriving (Show)

    box2list :: Box (ListF a) -> [a]
    box2list (Box s next) = case next s of
      Nil -> []
      Cons x s' -> x : box2list (Box s' next)

    runGen :: Cont Res () -> Box (ListF Int)
    runGen comp = Box ini step
      where
        ini () = runCont comp \() -> Stop
        step k = case k () of
          Stop -> Nil
          Res x k -> Cons x k

    data Res = Stop | Res Int (() -> Res)

    yield :: Int -> Cont Res ()
    yield x = cont $ Res x

    exampleGen :: [Int]
    exampleGen = box2list $ runGen do
      yield 1
      yield 2
      yield 3
\end{minted}

Корутины: stackfull/stackless (code coloring), first-class and not, symmetric/asymmetric.

Континуэйшены и full coroutines выражаются друг через друга.
...
\subsection{Эффективная реализация продолжений} \label{subsec:efficient-cps}

Существует несколько классических подходов к реализации продолжений первого класса на языковом уровне, которые, как правило, используются для построения генераторов или корутин.

\subsubsection{Continuous stack}
...
\subsubsection{Сегментный стек}

Стек вызовов представляется как связный список аллоцированных в куче сегментов, каждый соответствует ограничивающей операции. % todo cite
...
Таким образом, например, реализованы корутины в Kotlin, генераторы в C\#\footnote{\url{https://csharpindepth.com/Articles/IteratorBlockImplementation}}\ldots
