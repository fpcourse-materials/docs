...
\subsection{Концепция продолжения}
...
\subsubsection{Денотационная семантика с продолжениями} \label{subsubsec:denotational-cont}
...
Запишем теперь денотационную семантику для домена \mintinline{haskell}{K -> Int} как пару взаимно-рекурсивных функций:
\begin{minted}{haskell}
    evalK :: Expr -> K -> Int
    evalK e k = case e of
      Const n -> k `appK` n             -- выполнить остаток программы
      Diff l r -> evalK l (LDiff r : k) -- запомнить дальше вычислить правое поддерево

    appK :: K -> Int -> Int
    appK k result = case k of
      [] -> result                                -- дальше делать нечего
      LDiff r : k' -> evalK r (RDiff result : k') -- идем вычислять вправо
      RDiff l' : k' -> k' `appK` (l' - result)    -- вычисляем продолжение на результате
\end{minted}
...
То есть \texttt{evalDirect} лишь делегирует работу с продолжениями определяемого языка мета-языку, но каким образом?
Заметьте, что полученная в итоге реализация \texttt{evalK} хвостово-рекурсивная, а значит, может быть скомпилирована в цикл, не потребляющий аппаратный стек, при этом \mintinline{haskell}{K} это стек.
Таким образом, в первом случае мы делали рекурсивные вызовы и продолжение аллоцировалось как (аппаратный) стек мета-языка, а во втором случае мы самостоятельно аллоцируем стек в куче.
...
Чтобы получить в коде продолжение первого класса, нужно либо написать код в специальном виде (см.\ далее\ \ref{subsec:cps}), либо воспользоваться встроенным в язык оператором, которых придумано великое множество~\cite[приложение A]{hillerstrom2022foundations}.
...
\begin{minted}{haskell}
    ...
    data Frame
      ...
      | SetEnv Env -- после исполнения замыкания нужно вернуть первоначальный контекст
\end{minted}
...
Нужно не забыть после исполнения тела функции восстановить изначальный контекст для исполнения продолжения:\footnote{Для удобства буддем передавать окружение неявным параметром.}
...
\subsection{Continuation-passing style (CPS)} \label{subsec:cps}

% todo higher-order CPS

Если операция получения текущего продолжения не встроена в язык, текущее продолжение всё равно можно получить, переписав программу в \vocab{continuation-passing style (CPS)}.
...
А значит, интерпретатор полученный нами ранее (\ref{subsubsec:denotational-cont}) является CPS версией обычного интерпретатора, только с дефункционализированными продолжениями (они удобны для отладки~--- их можно распечатать, в отличие от функций Haskell).
Если расширить тот наш язык и написать на нём факториал, а потом сделать fusion (рефункционализацию дерева программы), мы получим текущую реализацию факториала.
