...
Для примера, запишем её для примитивного языка с вычитаниями:
...
Поэтому семантику редукционных контекстов обычно задают как синтаксис продолжений, перечень главных редукций и единственное правило вывода --- шаг в контексте.
%\[
%    \begin{array}{lrl}
%        \text{Terms} & t & \Coloneqq \mathbb{Z} ~|~ t \ominus t \\
%        \text{Evaluation context} & E & \Coloneqq \square ~|~ E \ominus t ~|~ \mathbb{Z} \ominus E \\
%        \text{(diff)} & & \mathbb{Z} \ominus \mathbb{Z} \longrightarrow \mathbb{Z} - \mathbb{Z}
%    \end{array}
%\]
...
\subsubsection{Продолжения первого класса}
...
% todo про концепцию продолжений, first-class продолжений
...
%Это наблюдение лежит в основе популярного формального метода задания операционной семантики~--- \vocab{reduction semantics}\footnote{\url{https://en.wikipedia.org/wiki/Operational_semantics}}.
%
%
