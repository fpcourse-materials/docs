...
В Haskell есть встроенная возможность объявлять новые типы данных на основании других типов, а также создавать их экземпляры.
...
\begin{minted}{haskell}
    ...
    describeAnimal animal = case animal of
...
\end{minted}
Haskell генерирует функции-аксессоры для доступа к полям значения:
\begin{minted}{haskell}
...
\begin{minted}{haskell}
    ...
    safeSqrtD :: Double -> MaybeD
    safeSqrtD x = if x < 0 then NothingD else JustD (calcSqrt x)
\end{minted}

Можно заметить, что так нам придётся объявлять отдельный тип \texttt{MaybeT} для каждого типа \texttt{T}.
...
\begin{minted}{haskell}
    ...
    safeSqrt x = if x < 0 then Nothing else Just (calcSqrt x)
\end{minted}
