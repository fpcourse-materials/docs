...
Рассмотрим построение алгебраической записи по структуре типа:
\begin{itemize}
    \item \mintinline{haskell}|Unit| записывается как $1$.
    \item \mintinline{haskell}|Bool| записываем как $1 + 1$ (рассматриваем как размеченное объединение двух \mintinline{haskell}|Unit|'ов).
    \item Используем удобные обозначения $2 \equiv 1 + 1, 3 \equiv 1 + 1 + 1$ и т.д.
    \item В целом суммы записываем через значок $+$: для \mintinline{haskell}|Either a b| записываем $a + b$.
    \item Для произведений используем значок $\times$: для \mintinline{haskell}|(a, b)| записываем $a \times b$.
    \item Для экспонент используем возведение в степень: для \mintinline{haskell}|a -> b| записываем $b^a$.
\end{itemize}

Рассмотрим немного примеров:
\begin{itemize}
    \item \mintinline{haskell}|Int8| записываем $256$.
    \item \mintinline{haskell}|Maybe a| записываем $1 + a$.
    \item \mintinline{haskell}|(Bool, Either Bool a)| записываем $2 \times (2 + a)$.
    \item \mintinline{haskell}|(a, b) -> c| записываем $c^{a\times b}$.
\end{itemize}
