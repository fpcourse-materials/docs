...
Haskell не позволяет создавать функции на типах по месту с помощью явной типовой лямбды\footnote{\url{https://stackoverflow.com/questions/4069840/lambda-for-type-expressions-in-haskell}} ввиду проблематичности этой конструкции для вывода типов. % todo ссылка далее
Однако полноценные функции на типах есть, и мы рассмотрим их далее. % todo ссылка далее
...
Далее мы рассмотрим технику, позволяющую типы высших порядков закодировать в языке, их не поддерживающем. % todo ref
...
\subsubsection{Обобщённые алгебраические типы данных (GADTs)}
...
\begin{task}
    Запишите определение \mintinline{haskell}{Maybe} в синтаксисе GADT.
\end{task}

С помощью GADT, в частности, мы можем задавать более точные возвращаемые типы конструкторов.
Так, мы говорим, что программа созданная с помощью конструктора \mintinline{haskell}{Const} вычисляется в число, с помощью \mintinline{haskell}{IsZero} вычисляется в булево значение по числу, а условное выражение требует булево значение в качестве условия, а типы веток и результата совпадают:
\begin{minted}{haskell}
    data Expr ty where
      Const :: Int -> Expr Int
...
Далее мы рассмотрим как GADT в Haskell выражаются через более базовые механизмы языка. % todo ref
...
\begin{itemize}
    ...
    \item[\positive] Легко на этапе компиляции отрабатывают \texttt{is}-проверки значений на принадлежность определённому типу (в остальных подходах с этим всё сложно); % todo выпросить инфу про подтипизацию у Михаила
...
\begin{itemize}
    \item Если разрешить использовать значения в типах, инстанциация может использоваться как механизм вычислений на этапе компиляции. % todo ref специализация
...
\begin{figure}
    % todo инвертировать цвета картинок
    \centering
...
Своего рода реализация параметрического полиморфизма через специальный. % todo ref
...
Если позволить мономорфизацию по \mintinline{haskell}{RuntimeRep} параметрам, получится система аналогичная гибридной реализации параметрического полиморфизма~\ref{subsubsec:hybrid}.
