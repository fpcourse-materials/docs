...
% todo параметрический полиморфизм в домашку
...
$\lambda$-абстракция позволяет обобщать выражения по значениям, каждая абстракция добавляет стрелку в тип выражения, аппликация же снимает стрелку:
\[
    \begin{array}{cc}
    ...
    \end{array}
\]
В то же время $\Lambda$-абстракция позволяет обобщать выражения по типам, добавляя квантор в тип ($\Pi$-абстракцию), а применение терма к типу (\vocab{универсальная аппликация (universal application)}) позволяет выбрать, с каким конкретным типом этот терм планируется использовать~\cite[глава 23]{pierce2002types}:
\[
    \begin{array}{cc}
...
Таким образом, например, функция $id$ фактически принимает \point{два аргумента: тип и значение}:
\[
    \begin{array}{ll}
...
Однако именованные функции на типах есть, и мы рассмотрим их далее~\ref{subsec:families}.
...
Иногда прокси-тип оставляют полиморфным, чтобы не зависеть от \mintinline{haskell}{Data.Proxy}.
...
Далее мы рассмотрим технику, позволяющую типы высших порядков закодировать в языке, их не поддерживающем (см.~\ref{subsubsec:simulating-hkt}).
...
Сейчас язык кайндов, который должен контролировать типы, слишком беден.
Действительно, кайнд \mintinline{haskell}|Suc| --- \mintinline{haskell}|Suc :: Type -> Type|, соответственно ничто не мешает написать, например, \mintinline{haskell}|Suc (Maybe Int)|.
...
Рассмотрим тип \href{https://hackage.haskell.org/package/tagged-0.8.8/docs/Data-Tagged.html#t:Tagged}{\mintinline{haskell}|Tagged|}, позволяющий дополнить тип значения дополнительным типовым тегом.
...
\begin{figure}[ht]
    ...
    \caption{Типы и кайнды --- одно (\cite{bragilevsky-haskell}).}
...
\subsubsection{Мономорфизация} \label{subsubsec:monomorphization}

\vocab{Мономорфизация}~--- самый прямолинейный подход: полиморфные функции и структуры компилируются для каждого набора типовых аргументов.
...
\begin{itemize}
    ...
    \item[\negative] В интерфейсах не может быть полиморфных методов, так как мы не знаем в месте вызова, к какому именно наследнику относится вызываемый метод, и какой код нужно специализировать (аналогично, не работает higher-rank полиморфизм);
...
Некоторые языки (например, C++ и Zig) не делают инстанциацию скрытой деталью реализации языка, а предоставляют её как инструмент пользователям.
...
\begin{itemize}
    ...
    Однако тогда функции для тестирования придётся вручную инстанцировать для всевозможных типов, иначе нельзя будет статически понять, компилируется ли она хотя бы для этих типов.
\end{itemize}
...
Агрессивный инлайнинг вместе с другими оптимизациями тоже помогают~\cite{blanchet1999escape}.
Java, например, также пытается аккуратно двигаться в сторону возможности мономорфизации\footnote{\href{https://youtu.be/JI09cs2yUgY?si=MLkRs31mN1koXIu1}{Type Specialization of Java Generics - What If Casts Have Teeth?}}\footnote{\url{https://cr.openjdk.org/~jrose/values/parametric-vm.html}}.
...
То есть следы дженериков в таком подходе есть и в промежуточном представлении CIL, и в рантайме.
