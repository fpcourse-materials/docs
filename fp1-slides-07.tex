...
\begin{document}
    ...
    \begin{frame}[fragile]{Правая свёртка списка или \ldots\footnote{За картинки спасибо Михаилу Беляеву.}}
        ...
        \vspace{0.5em}
        \begin{itemize}
            \item[\eg] \mintinline{haskell}|foldr f z [a, b, c] #$\equiv$# f a (f b (f c z))|
            \item[\then] То есть списки Чёрча --- свёртка \mintinline{haskell}|data| списков
            \begin{itemize}
    ...
    \begin{frame}[fragile]{Задачки на правую свёртку списков}
        ...
        \begin{itemize}
            ...
            \item[\todo] Реализуйте группировку пользователей по уровню подписки (\mintinline{haskell}|Data.Map.insertWith :: (a -> a -> a) -> k -> a -> Map k a -> Map k a|)
            \item[\answer] \pause
             \begin{minted}{haskell}
                 prod xs ys = foldr (+) 0 $ zipWith (*) xs ys
             \end{minted}
            \item[\answer] \pause
            \begin{minted}{haskell}
    ...
    \sectionplan{Ленивость и выбор свёртки \practicalslide}
