...
\end{minted}
Можно это значение принять в функции \texttt{f}, тогда оно автоматически пропагируется в \texttt{x}:
\begin{minted}{haskell}
    f :: Show b => (Show b => b) -> b
    f x = ?\framebox{x}?
\end{minted}
...
\begin{minted}{haskell}
    ...
    f d x = case d of ShowDict -> ?\framebox{x}? -- в области видимости доступен инстанс Show b
\end{minted}
...
В действительности вывод инстансов~--- это ни что иное, как \point{задача населения типа}.
...
То есть у нас имеется словарь \mintinline{haskell}|d1 :: ShowDict a|, а также функция \mintinline{haskell}|d2 :: ShowDict a -> ShowDict [a]|, пришедшая из импортов\footnote{Инстансы можно импортировать пустым импортом: \mintinline{haskell}|import Module ()|.}.
Необходимо сконструировать терм типа \mintinline{haskell}|ShowDict [a]|.
Очевидно, это будет просто аппликация одного к другому: \texttt{d2 d1}.
...
Так, например, работает \vocab{constraint trick}\footnote{\url{https://chrisdone.com/posts/haskell-constraint-trick/}}, позволяющий разрешать ad-hoc-полиморфные функции в параметрически-полиморфном контексте.
...
Параметры функций могут быть помечены ключевым словом \mintinline{scala}|implicit|, тогда Scala попытается их вывести самостоятельно с помощью доступных в области видимости \mintinline{scala}|implicit| деклараций.
...
\begin{minted}{scala}
    ...
    // Объект-синглтон, значение для пачки функций.
...
Помимо инъективности, классы типов также не обязательно обладают свойством \vocab{generativity}, критически важным для вывода типов, --- один и тот же результат не обязательно получен из одного и того же семейства:
\begin{minted}{haskell}
    f a ?$\sim$? g a ?$\Rightarrow$? f ?$\sim$? g
\end{minted}

Вместе инъективность и generativity --- \vocab{matchability}.
...
%Также, как и типы, констреинты параметризованы представлением\footnote{\url{https://hackage.haskell.org/package/base-4.21.0.0/docs/GHC-Exts.html\#t:CONSTRAINT}}: % todo пояснить зачем это
%\begin{minted}{haskell}
%    data CONSTRAINT (a :: RuntimeRep)
%    type Constraint = CONSTRAINT LiftedRep
%\end{minted}

% todo Типы кайнда Constraint не выводятся
...
В Haskell экзистенциальный тип выражают через \mintinline{haskell}{data} декларацию, в которой полиморфный конструктор данных и мономорфный тип\footnote{Подобно идиоме type erasure в C++.}.
Так, тривиальный экзистенциальный тип $\exists \alpha\ldotp \alpha$ можно получить в Haskell в виде типа \mintinline{haskell}{Any}, заданного следующим образом:
\begin{minted}{haskell}
    data Any where
...
Современный Haskell является синтаксически богатым языком, который, однако, несмотря на многообразие конструкций, транслируется в маленький типизированный внутренний язык.
Это язык $System~F_C$~\cite{sulzmann2007system}, расширяющий $System~F$ (\ref{subsec:lang-parametric-polumorphism}) несинтаксическими эквивалентностями типов.
...
А именно, вводится встроенный констреинт $\sim$, свидетельствующий об эквивалентности двух типов\footnote{\url{https://ghc.gitlab.haskell.org/ghc/doc/users_guide/exts/equality_constraints.html}}.
...
Чтобы получить информацию о типе, в области видимости должен быть инстанс \mintinline{haskell}|Typeable| для этого типа.
...
А именно: любой тип может быть преобразован в \href{https://hackage.haskell.org/package/base/docs/Data-Dynamic.html}{\mintinline{haskell}|Dynamic|}, а потом безопасно преобразован обратно.
\begin{minted}{haskell}
    data Dynamic where
...
Как мы обсуждали ранее, свойство когерентности гарантирует, что каждому типу в Haskell соответствует ровно один инстанс определённого класса типов.
...
Чтобы воспользоваться нестандартным инстансом класса типов для некоторого типа \texttt{a}, нужно объявить новый тип (например, с помощью \mintinline{haskell}|newtype|), содержащий данный, и написать для него нужный инстанс (см., например, \href{https://hackage.haskell.org/package/base-4.20.0.1/docs/Data-Ord.html#t:Down}{Down}).
...
\subsubsection{Открытые структуры} \label{subsubsec:open-structs}
