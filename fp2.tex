...
\begin{document}
    ...
    В общем случае, свёртку структуры данных уже можно рассматривать как интерпретацию некоторого языка~\cite{gibbons2014folding}.
    ...
    % todo java code reflection link
    % todo
    ...
    \begin{minted}{haskell}
        example1, example2 :: Lam String
        example1 = Lam $ Nothing `App` Just "y"                                 -- ?$\lambda x\ldotp x \ap y$?
        example2 = Lam $ Lam $ Nothing `App` Just Nothing `App` Just (Just "z") -- ?$\lambda x \ap y\ldotp x \ap y \ap z$?
    \end{minted}
    ...
    Если вложенная функция имела свободные переменные, ссылающиеся на локалы, добавляем глобальной функции аргументов по их числу.
    \begin{minted}{kotlin}
        fun f(x) {
            fun g(y) = x + y
            return g
    ...
    Теперь мы можем функциональные объекты представлять как пару
...
% todo   Таким образом, разрешение имён, resolve, нужно произвести статически один раз, ускорив дальнейшее исполнение программы.

    % todo stable pointers

    % todo подстановка

    % todo mention peter landin, closures
    ...
    % todo scope tower & resolve

    % todo 700 PLs

    % todo ссылка на Саймона
