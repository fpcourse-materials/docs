...
\begin{document}
    ...
    \begin{center}
    {\LARGE ФП 2.0}
        \footnote{Автор Андрей Стоян (\texttt{andrey.stoyan.csam@gmail.com}).}\footnote{Спасибо Илье Колегову за первое внимательное прочтение и кучу комментариев. Спасибо моим студентам, которые своим интересом к предмету сделали эту работу возможной.}\footnote{Версия от \today, \currenttime.}
        \\
        осень 2025
    \end{center}
    ...
    Этот подход оказывается очень полезным для изучения языков, построения выразительных языковых конструкций, а также является основой для продуктивного стиля программирования.
    ...
    В качестве основного языка курса выбран Haskell, так как он, с одной стороны, воплощает в себе многие концепции, часто доведённые до некоторого логического завершения, и достаточно выразителен для кодирования других.
    ...
    Так, детали реализации в GHC или теоретико-категорные основания вещей могут даваться в общем виде и без конкретики.
    ...
    \section{Эффекты и модульные интерпретаторы} \label{sec:effects}
    ...
    \section{Системы эффектов TODO} \label{sec:effect-systems}
    ...
    \section{Персистентность и оптика TODO}
...
%    \clearpage
    ...
    % todo заключение со ссылками на всё
...
%    \input{fp2-ghc-internals}
    ...
    % todo coroutines & structured concurrency

    % todo получше про defunctionalization, shallow, fusion...

    % todo "раздел, глава, параграф"

    % todo разметка ссылок на статьи, чтобы контролировать распределение внимания
