...
\begin{document}
    ...
    \subsection{Реализация классов типов}
    ...
    Неявные аргументы берутся из скоупа и подставляются автоматически:
    ...
    Действительно, после трансляции в Core, классы типов представляют собой словари функций.
    ...
    В общем случае процесс населения типа неразрешим.
    Поэтому GHC накладывает большое количество ограничений на фид инстансов, которые гарантируют тотальность вывода.
    ...
    Написание инстансов похоже на паттерн-матчинг времени компиляции по типу.
    Вывод инстансов выполнит эти мнимые \texttt{case-of} по типу и построит терм, который представляет собой вычисление, составленное из веток такого паттерн-матчинга.
    Таким образом, мы можем опускать информацию из типов в термы:
    \begin{minted}{haskell}
        type data Nat = Zero | Suc Nat
    ...
    Вывод инстансов опирается только на вид "головы" декларации --- части после \mintinline{haskell}|=>|.
    Это можно использовать, откладывая применение эквивалентностей.
    Так, например, работает constraint trick\footnote{\url{https://chrisdone.com/posts/haskell-constraint-trick/}}.
    ...
    \cite[глава 12]{maguire-types}

    % todo maguire custom type errors
    ...
    \subsubsection{Имплиситы}



    \footnote{\href{https://youtu.be/hIZxTQP1ifo?si=aG2Lk2eb-5E5SOLb}{Edward Kmett - Type Classes vs. the World.}}

    \cite{oliveira2010type}

    % todo

    % todo техническое мясо про вывод и формализмы
    % todo специализация

    \subsubsection{Правила и специализация}

    % todo

    \subsection{Семейства типов, ассоциированные семейства}
