...
\begin{document}
    ...
    Программный интерпретатор $U_M^N$ --- это программа на языке $M$, получающая на вход программу на языке $N$ и вход для неё --- данные из $D$, и возвращающая результат выполнения этой программы на этих данных: \[U_M^N : N\times D\to D\]
    ...
    И так далее.
    Получаем \emph{башню интерпретаторов}, на вершине которой находится язык, на котором мы хотим уже решать непосредственно нашу задачу:
    \[
        d_{out} =
        U^{x86}\left(\left<
        U_{x86}^{Asm}, \left<
    ...
    Использование функций является примером embedded domain specific language, когда мы вместо того, чтобы делать новый отдельный язык, мы его реализуем как библиотеку для уже существующего языка\footnote{\url{https://en.wikipedia.org/wiki/Domain-specific_language}}.
    ...
    % todo power of possibility makiing own language
    % todo vocab
    ...
    Так, операционная семантика задаёт смысл конструкций языка, в терминах некоторого более простого языка.
    ...
    Однако формальность такого определения будет зависеть от формальности описания семантики самого языка реализации интерпретатора~--- мета-языка.
    Такие интерпретаторы называют ``определяющими'', они задают семантику языка, жертвуя эффективностью ради наглядности.
    Взаимоотношения определяемого языка и мета-языка изучаются в классических статьях~\cite{reynolds1972definitional},\cite{reynolds1998definitional}.

    % todo denotational semantics

    % todo link to Oleg's a case for semantics

    % todo domain theory и семантика вообще, базовые интерпретаторы

    % todo

    \subsection{Инициальные интерпретаторы: с тегами и без}
