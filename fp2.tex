...
\begin{document}
    ...
    \begin{center}
    {\LARGE ФП 2.0}
        \footnote{Автор Андрей Стоян (\texttt{andrey.stoyan.csam@gmail.com}).}\footnote{Особая благодарность Илье Колегову за первое внимательное прочтение и кучу комментариев.}
        \\
        осень 2025
    \end{center}
    ...
    \section{Общие знания о типах данных} \label{sec:data-common}
    ...
    \section{Данные по-другому} \label{sec:curious-data}

    \input{fp2-curious-data}
    ...
    \section{Классические интерпретаторы} \label{sec:classical-interpreters}
...
%    \clearpage
    ...
    \section{Эффекты, хендлеры и трансформеры} \label{sec:effect-handlers}
