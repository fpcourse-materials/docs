...
\begin{document}
    ...
    % todo equality constraints paper
    % todo GADTs paper
    ...
    % todo дефункционализация, слайды откопать про это всё
    ...
    % todo links in defunctionalization slides
    ...
    \begin{minted}{haskell}
        fun <M> Monad<M>.doMonad(x: Apply<M, Int>): Apply<M, Int> =
    ...
    Как мы уже обсуждали выше~\ref{subsubsec:type-erasure}, параметрический полиморфизм в Haskell реализуется следующим образом: все значения хранятся в куче и передаются в полиморфные функции по ссылке.
...
%    % todo
    ...
    % todo где-то должно быть содержание книжки про типы Sandy Maguire, data femilies
    ...
    % todo где-то нужно про систему кайндов, константы, полиморфизм...
